...
\section{Actions}
Actions are submitted to the world using a secure method that ensures that the sender is actually from the owner of the private key.

%Traditionally player actions are held on the servers of the game provider. In the effort of preserving data, a player "state" is kept, which contains the player location, health, items, etc. This works well for the purposes of a proprietary game, but will not work for this system. It is too difficult to properly keep track of a player state with so many different worlds. You run the risk of worlds cheating and player abuse. It lacks traceability and there is no way for world to scale experience. 
...
%	- Experience packages are of the form
...
\section{Items} 
\label{items}
Items are defined and generated by worlds. They have transferability to other other worlds only if the two worlds have reached an agreement on their exchangability.

\subsection{Item Listing}
An items listing is a data construct that lives at beginning of the item ledger (\ref{IL}) for the item. Figure \ref{IList} shows an example of a possible item listing. The values are defined in figure \ref{ILexp}.
