...
%		- Players can state EXP as well when they enter a world
...
% -> Still have adjacent worlds.
% -> Transport is more chill.
%	-> How to prevent players from playing on more then one world at at time?
% -> Player location on WSC? This is a simple way of doing it.
...
\section{Experience}
\label{exp}
Experience is distributed to players using experience packages, defined as $e$. These are simply a package of the form (Figure \ref{exppkg}). When a player is to received experience this package is hashed, signed and sent to the \textit{Worlds Smart Contract} (WSC) \ref{WSC}. The world must state some WOR in order to grant experience, the effect of the experiance of the player is directly proportional to the amount of WOR that is staked. Experience is distributed very similar to items, although it is not transferable. It is possible for experience to be liquidated back into WOR.

\subsection{Staking Experience}
Some worlds might require you to stake some experience before entering. The experience enters a special state where it is owned by neither the player or the world. The world is required to send a signed "pulse" for the player to the WSC to indicate that the player is still alive. If the player is to die, the experience will fall under ownership of the world. If the world is PVP then it might distribute the EXP to the player that killed the other player, or the world might keep the experience for redistribution.

\begin{figure}[H]
\centering
\label{exppkg}
\caption{Experience Package $e$ - Sword Fighting}
\begin{lstlisting}
...
\label{WSC}
The worlds smart contract is a contract that form consensus among the network. It contains functions and data elements that are used as various forms of proof that require interoperability between worlds. 
